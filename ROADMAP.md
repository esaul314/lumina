...
Implementation note (2026-07-20): the latest Step 4 slices route simple, photo, feed, pool, playback, and `patch-state` commands through one indexed reducer-family interpreter with explicit `now`/`rng` environment adapters, and capture the dispatcher effect interpreter once before its promise-reduce sequence. Unsupported and inherited object keys, plus unhandled effects, remain unchanged/no-op boundaries.
