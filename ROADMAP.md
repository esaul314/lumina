...
- Next: continue the Phase 1 implementation companion with Step 4, focused on extending the new reducer-local combinators, shared dispatch-route shell, dispatcher readability patterns, route-decode algebra, and declarative listener/command specs only where they remove real repeated command/effect ceremony. The latest slice standardized transport command decoding into shared required-value decoder specs for photo and pool mutations, so `server/domain/commands.js` now reads as small normalization combinators plus command-local payload policies while preserving the existing wire contracts. The broader Step 4 readability pass remains active and intentionally selective.
