...
- Next: continue the Phase 1 implementation companion with Step 4, focused on extending the new reducer-local combinators, shared dispatch-route shell, dispatcher readability patterns, route-decode algebra, and declarative listener/command specs only where they remove real repeated command/effect ceremony. The latest slice centralized the durable socket state-patch event specs in the shared command module, so `server/sockets.js`, the legacy fallback, and domain-level tests now reuse one declarative `event -> patch builder -> shared patch-state command` boundary instead of rebuilding that wiring locally. The broader Step 4 readability pass remains active and intentionally selective.
