...
- In progress: implementation companion Step 5 has started with pure, partially applicable client state-patch builders shared by the REST action hook.
- Next: continue Step 5 by aligning the remaining client request/action contracts and snapshot projections, with explicit JSDoc seams that can migrate mechanically to TypeScript.
