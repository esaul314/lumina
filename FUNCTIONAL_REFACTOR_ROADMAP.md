...
- The latest UI refinement adds `client/src/state/imageFeedsPanels.js`, where panel identifiers and immutable collapse/maximize transitions form a small JSDoc-typed state algebra; the React component interprets those transitions and CSS interprets focused versus two-panel layout declaratively.
- Drag-reordering is intentionally deferred from this slice: without persisted order semantics it would be cosmetic, and pointer capture would compete with the rating deck's existing crop gesture. A future reorder slice should begin with a pure ordered-panel projection and keyboard/touch interaction contract.
