...
- The latest Step 4 slices collapsed the remaining standalone REST single-command registrations in `server/routes.js` onto one local method-aware spec table, then collapsed the remaining socket command-listener registration ceremony onto one shared listener-family table, and now align the overlapping pool keyword/feed-config REST patch specs with their durable socket command specs through one shared pool transport family, so the route shell keeps its explicit `decode -> guard -> dispatch -> present` behavior while `server/sockets.js` now reads as one small interpreter over shared listener metadata plus the explicit transport-only tail.
