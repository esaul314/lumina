...
10. **Develop/Run Tests in the Correct Target Host (Playwright)**: Lumina integration tests that require browsers (like Playwright tests) should be executed on the `playwright` host (`alex@playwright`), as the local host environment (filament) lacks GUI libraries and dependencies like Chrome. Avoid testing in the wrong execution host.
