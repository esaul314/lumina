...
- Implementation companion Step 4 complete: the backend command/effect readability pass meets its acceptance criteria. A quality audit found the broader client boundary still needed the same standard; Step 5 is active with pure state-patch builders, declarative REST-first mutation request plans, and a shared JSON transport boundary.
...
- RemoteControl live acknowledgements should use pure status projections plus declarative subscription records when event branches share policy; credential-specific input clearing remains in the view effect shell.
